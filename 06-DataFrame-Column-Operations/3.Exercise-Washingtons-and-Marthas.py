...
washingtonsCache = (spark
  .read
  .parquet(parquetDir)
  .cache()
) 

washingtonsFilter = (washingtonsCache
  .filter( col("project") == "en" )
)

washingtonsEndsWith = (washingtonsFilter
  .filter( col("article").endswith("_Washington") == True)
)

washingtons = (washingtonsEndsWith
  .collect()
)
...
marthasFilter = (marthasCache
  .filter( col("project") == "en" )
)

marthasStartsWith = (marthasFilter
  .filter( col("article").startswith("Martha_") == True)
)

marthas = (marthasStartsWith
  .collect()
)
