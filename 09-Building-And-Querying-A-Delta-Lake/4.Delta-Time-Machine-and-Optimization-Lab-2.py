...
# MAGIC %md
# MAGIC # Delta Lake Time Machine & Optimization Lab
# MAGIC 
# MAGIC Delta Lake allows you to read, write and query data in data lakes in an efficient manner.
# MAGIC 
...
# MAGIC 
# MAGIC If you get an error from either of the next two SQL queries, running the solution code for the "Delta-Lake-Lab-1" will build all necessary tables.
...
# TODO
count = spark.sql( "SELECT COUNT(*) FROM customer_data_delta WHERE country = 'Sweden'" ).collect()[0][0]
print(count)
...
# TODO
%timeit preZorderQuery = spark.sql("SELECT * FROM customer_data_delta").collect()
...
# TODO
%timeit postZorderQuery = spark.sql("SELECT * FROM customer_data_delta").collect()
