...
evalFirstName = (txtDF
                .filter( col("firstName") == "CAROL")
                )

# display(evalFirstName)

lowercaseTxtDF = (txtDF
                 .withColumn("firstName", lower( col("firstName") ) )
                 .withColumn("middleName", lower( col("middleName") ) )
                 .withColumn("lastName", lower( col("lastName") ))
                 .withColumn("gender", lower( col("gender") ))
                 .withColumn("ssn", regexp_replace("ssn", '-', ''))
                 .distinct()
)

countingLowercaseTxtDF = lowercaseTxtDF.count()

print(countingLowercaseTxtDF)
#display(lowercaseTxtDF)

# COMMAND ----------

destFile = userhome + "/people.parquet"

(lowercaseTxtDF.write
